...
function Cart({cart, setCart}) {
  ...
  function changeQuantity(id, type){
    ...
    const updatedOrder = type === "increase" ? {...cart[index], quantity: cart[index].quantity + 1} : {...cart[index], quantity: cart[index].quantity === 0 ? 0 : cart[index].quantity - 1};
    const newCart = [
      ...cart.slice(0, index),
      updatedOrder,
      ...cart.slice(index + 1)
        ...
        <tbody className='font-normal'>
          ...
          <tr key={product.id}>
            <td className="flex gap-4 items-center pb-5">
              <img src={products[product.id - 1].image} alt={products[product.id - 1].title} className="w-10"/>
              <Link to={'/shop/product/' + product.id}>
                <p className="hover:underline">{products[product.id - 1].title}</p>
              </Link>
            </td>
            <td className="text-center pb-5">
              {products[product.id - 1].price} &#8364;
            </td>
            <td className="pb-5">
